...
DataCleaner는 파싱 실패를 최대 3패스 재요청하고, 남은 논문은 attri 없이 유지한다(outline 매핑은 title·abstract로 되므로 196편 전부 매핑됨). 실패는 llama가 만든 JSON의 문법 오류라 원문 형식과 무관하다. **두 실행 모두 논문 과반이 속성 트리 없이 본문 생성에 들어갔다.**

**사후 분석(같은 날)**: 파일럿 논문 12편에 attri 프롬프트를 다시 보내 원본 응답을 받아 보니 실패 10건이 **전부 같은 자리**였다. 프롬프트의 출력 예시 자체가 `"other info": [ "info1": "", "info2": {…} ]`(배열 안에 키-값)로 잘못돼 있고 llama는 이를 그대로 따른다(gpt-4o는 알아서 고쳤을 것). 결정적 복구(`repair_json_text`: `[ "키":` 괄호 쌍 → 객체, 후행 쉼표 제거)로 10/10이 파싱됐고, 이를 `DataCleaner`에 넣었다(kisti-run.md §3). 프롬프트 원문은 유지.
...
1. **AttributeTree JSON 복구** — 넣기로 결정하고 적용(§4.2 사후 분석). 배치 첫 편에서 `run.json.attri.with_attri`로 효과를 확인한다.
